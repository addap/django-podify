...
TODO: add functionality to specify folder with mp3s to create a podcast from. Have to read up on forms for that
TODO: add set_invalid button to episodes (OR save youtube-id upon deletion so that they do not get readded in the next update)
